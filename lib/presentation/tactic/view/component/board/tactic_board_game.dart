...
abstract class TacticBoardGame extends FlameGame
  ...
  bool isAnimating = false;
  late DrawingBoardComponent drawingBoard;
  Future<void> addItem(FieldItemModel item);
  Future<void> loadScene(AnimationItemModel? scene);
  void redrawLines();
}
...
class TacticBoard extends TacticBoardGame
  ...
  void onTapDown(TapDownInfo info) {
    if (isAnimating) return;
    super.onTapDown(info);
    final tapPosition = info.raw.localPosition;
    ...
    if (components.isNotEmpty) {
      if (!components.any((t) => t is FieldComponent) &&
        ...
        try {
          final f = ref.read(boardProvider).selectedItemOnTheBoard;
          if (f is FreeDrawModelV2) {
            deselect = false;
          }
        } catch (e) {}
