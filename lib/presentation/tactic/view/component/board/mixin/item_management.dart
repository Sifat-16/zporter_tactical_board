...
mixin ItemManagement on TacticBoardGame {
  ...
  _addFreeDrawing({required List<FreeDrawModelV2> lines}) {
    if (children.whereType<DrawingBoardComponent>().isEmpty) {
      drawingBoard = DrawingBoardComponent(
        position: gameField.position,
        size: gameField.size,
        eraserStrokeWidth: 20.0,
    ...
    final duplicateLines = lines.map((l) {
      return l.clone()
        ..points = l.points
  ...
  @override
  Future<void> loadScene(AnimationItemModel? scene) async {
    ...
    if (children.whereType<DrawingBoardComponent>().isNotEmpty) {
      drawingBoard.resetDrawing();
    }

    if (scene == null) {
      zlog(data: "loadScene called with a null scene. Board cleared.");
      return;
    }

    zlog(data: "Loading new scene: ${scene.id}");
    addInitialItems(scene.components);
  }

  void addInitialItems(List<FieldItemModel> initialItems) async {
    for (var f in initialItems.where((i) => i is! FreeDrawModelV2)) {
      await addItem(f);
    }
    _addFreeDrawing(
      lines: initialItems.whereType<FreeDrawModelV2>().toList(),
    );
  }

  void addNewTextOnTheField({required TextModel object}) {
    object = object.copyWith(
      offset: SizeHelper.getBoardRelativeVector(
        gameScreenSize: gameField.size,
        actualPosition: gameField.size / 2,
    ...
    ref.read(animationProvider.notifier).addBoardItem(object);
  }
}
