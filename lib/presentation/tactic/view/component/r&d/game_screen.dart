...
class _GameScreenState extends ConsumerState<GameScreen> {
  late final TacticBoard tacticBoardGame;
  ...
  void initState() {
    ...
    zlog(data: "TacticBoard game created in initState.");

    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      ref.read(boardProvider.notifier).updateGameBoard(tacticBoardGame);
      ref.read(boardProvider.notifier).updateBoardBackground(
          widget.scene?.boardBackground ?? BoardBackground.full);
      ref.read(boardProvider.notifier).updateBoardColor(
            ref.read(animationProvider.notifier).getFieldColor(),
          );
    });
  ...
  void didUpdateWidget(covariant GameScreen oldWidget) {
    ...
    final hasSceneChanged = oldWidget.scene?.id != widget.scene?.id;
    final isUndoOperation = ref.read(animationProvider).isPerformingUndo;

    if (hasSceneChanged || isUndoOperation) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        tacticBoardGame.loadScene(widget.scene);
  ...
  @override
  Widget build(BuildContext context) {
    ref.watch(animationProvider.select((s) => s.selectedScene?.id));
    final bp = ref.watch(boardProvider);
    int quarterTurns = bp.boardAngle;

    ref.listen(boardProvider.select((s) => s.refreshBoard), (previous, next) {
      if (next == true) {
    ...
    return DragTarget<FieldItemModel>(
      onAcceptWithDetails: (details) async {
        if (isBoardBusy(bp) || _currentExportDialogContext != null) {
          return;
        }
        ...
        final Vector2 cursorPosition;
        if (quarterTurns == 1) {
          final double gameScreenWidth = gameScreenSize.x;
          cursorPosition = Vector2(
              screenRelativeOffset.y, gameScreenWidth - screenRelativeOffset.x);
        } else {
          cursorPosition = screenRelativeOffset;
        }
        ...
        fieldItemModel.offset = SizeHelper.getBoardRelativeVector(
          gameScreenSize: tacticBoardGame.gameField.size,
          actualPosition: adjustedTargetPosition,
        );
        ...
        ref.read(animationProvider.notifier).addBoardItem(fieldItemModel);
      },
      builder: (BuildContext context, List<Object?> candidateData,
          List<dynamic> rejectedData) {
        return RotatedBox(
