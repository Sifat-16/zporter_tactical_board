...
class AnimationController extends StateNotifier<AnimationState> {
  ...
  // --- NEW CENTRAL DATA MANIPULATION METHODS ---

  void addBoardItem(FieldItemModel newItem) {
    final currentScene = state.selectedScene;
    if (currentScene == null) return;

    final newComponents = List<FieldItemModel>.from(currentScene.components)
      ..add(newItem);

    final updatedScene = currentScene.copyWith(components: newComponents);
    _updateStateWithNewScene(updatedScene);
  }

  void deleteBoardItem(FieldItemModel itemToDelete) {
    final currentScene = state.selectedScene;
    if (currentScene == null) return;
  ...
  void updatePlayerPosition(String playerId, Vector2 newRelativeOffset) {
    _updateComponent(
      componentId: playerId,
      updateLogic: (component) {
        if (component is PlayerModel) {
          return component.copyWith(offset: newRelativeOffset);
        }
        return component;
      },
  ...
  void _updateComponent({
    ...
    required FieldItemModel Function(FieldItemModel) updateLogic,
    bool shouldTriggerSave = true,
  }) {
    final currentScene = state.selectedScene;
    if (currentScene == null) return;

    final componentIndex =
        currentScene.components.indexWhere((c) => c.id == componentId);

    if (componentIndex != -1) {
      final newComponents = List<FieldItemModel>.from(currentScene.components);
      final updatedComponent = updateLogic(newComponents[componentIndex]);
  ...
  void _updateStateWithNewScene(AnimationItemModel newScene,
      {bool shouldTriggerSave = true}) {
    if (shouldTriggerSave) {
      // saveCurrentChanges(); // Autosave can be re-enabled here if desired
    }

    state = state.copyWith(selectedScene: newScene);

    if (state.selectedAnimationModel != null) {
      final anim = state.selectedAnimationModel!;
      final sceneIndex =
          anim.animationScenes.indexWhere((s) => s.id == newScene.id);

      if (sceneIndex != -1) {
        final newScenes = List<AnimationItemModel>.from(anim.animationScenes);
        newScenes[sceneIndex] = newScene;
        final newAnim = anim.copyWith(animationScenes: newScenes);
        state = state.copyWith(selectedAnimationModel: newAnim);
      }
    }
  }

  Future<void> saveCurrentChanges() async {
    BotToast.showText(text: "Changes Saved!");
    // Your previous database saving logic from `_onAnimationSave` or `_onSaveDefault`
    // would go here. It would read from `state.selectedScene` directly.
  }

  // --- ORIGINAL METHODS (FULLY INTEGRATED) ---
  ...
  Future<void> saveAnimationTime({
    ...
    if (currentScene?.id == sceneId) {
      final updatedScene = currentScene!.copyWith(sceneDuration: newDuration);
      _updateStateWithNewScene(updatedScene);
    }
  }
  ...
  void deleteDefaultAnimation() async {
    ...
    try {
      ...
      // This might need adjustment. Clearing the provider directly might not be what you want.
      // The game should react to the state change instead.
      // ref.read(boardProvider.notifier).clearItems();
    } catch (e) {}
    BotToast.cleanAll();
  }
  ...
  void updateBoardColor(Color c) {
    AnimationModel? selectedAnimation = state.selectedAnimationModel;
    AnimationItemModel? selectedScene = state.selectedScene;
    selectedAnimation = selectedAnimation?.copyWith(fieldColor: c);
    selectedScene = selectedScene?.copyWith(fieldColor: c);
