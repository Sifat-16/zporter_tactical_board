...
class BoardController extends StateNotifier<BoardState> {
  BoardController(this.ref) : super(const BoardState());

  final Ref ref;

  void toggleSelectItemEvent({
    required FieldItemModel? fieldItemModel,
    String? camefrom,
  }) {
    final previousSelectedModel = state.selectedItemOnTheBoard;
    if (fieldItemModel == null) {
      state = state.copyWith(forceItemModelNull: true);
    } else {
      if (fieldItemModel.id == previousSelectedModel?.id) {
  ...
  void removeElement() {
    final itemToRemove = state.selectedItemOnTheBoard;
    if (itemToRemove != null) {
      ref.read(animationProvider.notifier).deleteBoardItem(itemToRemove);
      state = state.copyWith(forceItemModelNull: true);
    }
  }
  ...
  void rotateField() {
    final angle = state.boardAngle == 0 ? 1 : 0;
    state = state.copyWith(boardAngle: angle);
  }
  ...
  void editTextComponent({required TextModel textModel}) {
    ref.read(animationProvider.notifier).deleteBoardItem(textModel);
    ref.read(animationProvider.notifier).addBoardItem(textModel);
  }
}
