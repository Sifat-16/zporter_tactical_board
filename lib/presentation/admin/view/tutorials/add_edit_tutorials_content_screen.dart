...
class _TutorialEditorScreenState extends ConsumerState<TutorialEditorScreen> {
  ...
  Widget _buildQuillToolbar() {
    ...
    return QuillSimpleToolbar(
      ...
      config: QuillSimpleToolbarConfig(
        ...
        // --- THIS IS THE FIX ---
        // `customButtons` is a direct property of the config, not nested inside `buttonOptions`.
        customButtons: [
          QuillToolbarCustomButtonOptions(
            icon: Icon(Icons.format_line_spacing),
            iconTheme: iconTheme,
            onPressed: () {
              _showLineHeightDialog(context);
            },
          ),
        ],
        buttonOptions: QuillSimpleToolbarButtonOptions(
  ...
  void _showLineHeightDialog(BuildContext context) {
    final values = <double?>[null, 1.0, 1.5, 2.0];
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: values.map((value) {
                final isSelected = _quillController
                        .getSelectionStyle()
                ...
                return ListTile(
                  title: Text(
                    value == null ? 'Default' : value.toString(),
                    style: TextStyle(
                      color:
                  ...
                  onTap: () {
                    _quillController.formatSelection(
                        LineHeightAttribute(lineHeight: value));
                    Navigator.of(context).pop();
                  },
                );
              }).toList(),
...
// The _SmartMediaThumbnail widget remains the same as the last correct version.
class _SmartMediaThumbnail extends StatelessWidget {
  final String url;
  final String? tutorialThumbnail;
  const _SmartMediaThumbnail({required this.url, this.tutorialThumbnail});
